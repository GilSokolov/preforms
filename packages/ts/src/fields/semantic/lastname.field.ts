import { TextField, TextFieldConfigDerived } from "../inputs/text-field";

/**
 * Last name text field.
 *
 * A semantic wrapper around {@link TextField} that:
 * - Defaults the field key to `"lastName"`
 * - Is required by default
 * - Accepts either a raw string value or a full field configuration
 */
export class LastName extends TextField {
  /**
   * @param configOrValue - Initial last name value or full field configuration
  ...
  constructor(
    ...
    super({ key: "lastName", aria: { "aria-label": "Last Name" }, ...config });
  }
}
